...
namespace Xlnt.Data
{
    ...
    public class DataReaderExtensionsTests
    {
		...
		public void As_supports_automatic_field_matching() {
			...
			Verify.That(() => rows.Count == 1);
			Verify.That(() => rows[0] == new Row{ Id = 1, Value = items[0].Value });        
        }
		...
		public void As_ignores_missing_fields() {
            ...
            data.ColumnMappings.Add(x => x.Value);
            Verify.That(() => data.As<Row>().ToList().Single() == new Row { Value = items[0].Value });
        }
		...
		public void As_supports_writable_properties() {
            ...
            data.ColumnMappings.Add(x => x.Value);
            Verify.That(() => data.As<WrappedValue<string>>().ToList().Single() == items[0]);
        }
        ...
        public void As_ignores_read_only_properties() {
            ...
            data.ColumnMappings.Add(x => x.Value);
            Verify.That(() => data.As<WithReadOnlyProperty>().ToList().Single() == new WithReadOnlyProperty{ Value = 2 });
        }
        ...
        static void CheckRoundtrip<T>(Converter<IDataReader, Item<T>> toItem, params Item<T>[] items) {
			...
			items.ForEach((n, row) => Verify.That(() => row == rows[n])); 
        }
    }
}