...
type LinqQueryRewritingSession() =
    ...
    static let TablePattern = Regex(@"^(?<op>\s*(FROM|INNER JOIN|LEFT OUTER JOIN|CROSS JOIN) )(?<table>(, )?((\[.+?\]\.)?\[.+?\] AS \[.+?\]))+", RegexOptions.Compiled +  RegexOptions.Multiline + RegexOptions.ExplicitCapture)
